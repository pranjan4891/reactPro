...
const DataTable: FC<DataTableProps> = ({ locations, updateStatus }) => {
  const headers = ['Name', 'Email', 'Phone', 'Location', 'Department', 'Role', 'Venue', 'Active', 'Edit'];
  ...
  return (
    <table className="min-w-full divide-y-2 divide-gray-200 rounded-sm bg-white text-gray-200 text-sm">
      ...
      {<tbody className="divide-y divide-gray-200">
        {locations.map(({ _id, userName, email, phoneNumber, location, department, role,venue, isActive }) => (
          <tr key={_id}>
            <td className="whitespace-nowrap px-4 py-2 font-medium text-gray-900">{userName}</td>
            ...
            <td className="whitespace-nowrap px-4 py-2 text-gray-700">{department}</td>
            <td className="whitespace-nowrap px-4 py-2 text-gray-700">{role}</td>
            <td className="whitespace-nowrap px-4 py-2 text-gray-700">{venue}</td>
            <td className="whitespace-nowrap px-4 py-2 text-gray-700">
              ...
              >
                <input
                  ...
                  onChange={() => updateStatus(_id!, isActive)}
