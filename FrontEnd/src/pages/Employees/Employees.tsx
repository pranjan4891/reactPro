...
import { getEmployees, addEmployees, updateEmployee } from "../../utils/http-requests";
...
const Employees: FC = () => {
  ...
  const updateStatus = (id:string, currentStatus:string) =>{
    ...
    let data1:UpdateStatusInput = {id:id, data:{ isActive: newStatus }}
    updateStatusMutation.mutate(data1);  
  }

  const addLocationMutation = useMutation({
    mutationFn: (newEmployess: EmployeesIface) => {
      unsetGeneralError();
      return addEmployees(newEmployess);
    },
    onSuccess: () => {
      console.log('enter');
      ...
      unsetGeneralError();
      toast.success("Employee Saved Successfully!", { autoClose: 1500 });
      refetch();
    },
    onError: (error: any) => {
  ...
  const updateStatusMutation:any = useMutation({
    mutationFn: (data1:UpdateStatusInput) => {
      const {id , data} = data1;
      console.log(id, data);
      return updateEmployee(id, data); // Call your API function to update the status
    },
    onError: (error) => {
  ...
  const addLocation = useCallback((newEmployee: EmployeesIface) => {
    addLocationMutation.mutate(newEmployee);
  }, [addLocationMutation]);
    ...
    if (isLoading) {
      return (
        <div className="h-full px-4 text-center">
          <h6 className="py-5 text-md font-md text-gray-400">Loading Pickup Locations, Please wait...</h6>
    ...
    if (isError || data?.httpStatusCode !== 200) {
      return (
        <div className="flex justify-center pt-20">
          <FetchError message="Failed to fetch Pickup Locations." onRefresh={refetch} />
        </div>
      );
    }
  ...
  return (
    <div className="flex flex-1 flex-col bg-white shadow-lg p-2 pt-0 overflow-x-hidden rounded-md overflow-y-auto">
      ...
      <UpdateModal addEmployee={addLocation} mutatePending={addLocationMutation.isPending} error={generalError} isOpen={isModalOpen} onClose={closeModal}>
...
const Header: FC<{ openModal: () => void, success:boolean | undefined }> = ({ openModal, success }) => (
  <div className="flex py-1 justify-between items-center w-full">
    <h2 className="font-bold text-lg">
      <i className="ri-store-3-line"></i> Employee
    </h2>
    {success && (
    <div className="flex">
      <SearchBar />
