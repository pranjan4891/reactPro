...
import { getVenue, addVenue, updateVenue } from "../../utils/http-requests";
...
const Venue: FC = () => {
  ...
  const addLocationMutation = useMutation({
    mutationFn: (newLocation: VenueIface) => {
      unsetGeneralError();      
  ...
  const addVenues = useCallback((newLocation: VenueIface) => {
   ...
   const renderContent = () => {
    if (isLoading) {
      return (
        <div className="h-full px-4 text-center">
          <h6 className="py-5 text-md font-md text-gray-400">Loading Venue, Please wait...</h6>
    ...
    <>
        { data?.total ? (  <DataTable VenueResponse={data.data} updateStatus={updateStatus} />) : <NoRecordFound onRefresh={refetch} /> }
    </>
