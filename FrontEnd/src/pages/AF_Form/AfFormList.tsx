...
import { getAfform } from "../../utils/http-requests";
...
import AfformDataTable from './AfFormDataTable';
...
//     mutationFn: (newLocation: AfformIface) => {
//       unsetGeneralError();      
//       return addVenue(newLocation);
//     },
//     onSuccess: () => {
//       console.log('enter');
...
//       unsetGeneralError();
//       toast.success("Venue Saved Successfully!", { autoClose: 1500 });
//       refetch();
//     },
//     onError: (error: any) => {
...
//       const {id , data} = data1;
//       console.log(id, data);
//       return updateVenue(id, data); // Call your API function to update the status
//     },
//     onError: (error) => {
...
//   const addVenues = useCallback((newLocation: AfformIface) => {
...
//   }, [addLocationMutation]);
   ...
   const renderContent = () => {
    if (isLoading) {
      return (
        <div className="h-full px-4 text-center">
          <h6 className="py-5 text-md font-md text-gray-400">Loading AF Forms, Please wait...</h6>
    ...
    if (isError || data?.httpStatusCode !== 200) {
      return (
        <div className="flex justify-center pt-20">
          <FetchError message="Failed to fetch AF Forms ." onRefresh={refetch} />
        </div>
      );
    }
    ...
    <>
        {/* { data?.total ? (  <AfformDataTable AfFormResponse={data.data} updateStatus={updateStatus} />) : <NoRecordFound onRefresh={refetch} /> } */}
        { data?.total ? (  <AfformDataTable AfFormResponse={data.data} />) : <NoRecordFound onRefresh={refetch} /> }
    </>

    ) : 
    <div className="flex justify-center pt-20">
      <FetchError message="Failed to fetch Venue." onRefresh={refetch} />
    </div>;
  ...
  return (
    <div className="flex flex-1 flex-col bg-white shadow-lg p-2 pt-0 overflow-x-hidden rounded-md overflow-y-auto">
      ...
      <div className="flex flex-col h-full max-h-full overflow-y-auto w-full p-1">
          ...
          <div className={`flex-1 overflow-x-auto overflow-y-auto max-h-full bg-white h-full rounded-sm scroll-smooth`}>
            {renderContent()}
          </div>
          {data?.total && (
      ...
      {/* <UpdateModal addVenue={addVenues} mutatePending={addLocationMutation.isPending} error={generalError} isOpen={isModalOpen} onClose={closeModal}>
...
const Header: FC<{ openModal: () => void, success:boolean | undefined }> = ({ openModal, success }) => (
  <div className="flex py-1 justify-between items-center w-full">
    <h2 className="font-bold text-lg">
      <i className="ri-store-3-line"></i> Venue
    </h2>
    {success && (
    <div className="flex">
      <SearchBar />
