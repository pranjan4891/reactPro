...
interface DataTableProps {
  locations: LocationIface[];
  updateStatus: (_id: string, currentStatus: string) => void; // Ensure this matches the expected function signature
}

const DataTable: FC<DataTableProps> = ({ locations, updateStatus }) => {
  const headers = ['Name', 'Address', 'State', 'City', 'Active'];
  ...
  return (
    <table className="min-w-full divide-y-2 divide-gray-200 rounded-sm bg-white text-gray-200 text-sm">
      ...
      {<tbody className="divide-y divide-gray-200">
        {locations.map(({ _id, name, city, country, state, status }) => (
          <tr key={_id}>
            <td className="whitespace-nowrap px-4 py-2 font-medium text-gray-900">{name}</td>
            <td className="whitespace-nowrap px-4 py-2 text-gray-700">{state}</td>
            <td className="whitespace-nowrap px-4 py-2 text-gray-700">{city}</td>
